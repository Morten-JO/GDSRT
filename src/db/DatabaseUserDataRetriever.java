...
public class DatabaseUserDataRetriever implements IUserDataRetriever{
	...
	public boolean updateUserEyeLevel(String userId, int eyeLevel) throws SQLException {
		if(userExists(userId)) {
			PreparedStatement statementUpdate = dbController.prepareSafeStatement("update "+tableName+" set EYE_LEVEL = ? where USER_ID = ?");
	...
	public boolean updateUserAdditionalData(String userId, String additionalData) throws SQLException {
		if(userExists(userId)) {
			PreparedStatement statementUpdate = dbController.prepareSafeStatement("update "+tableName+" set ADD_DATA = ? where USER_ID = ?");
	...
	public boolean addUser(String userId, int eyeLevel, String additionalData) throws SQLException {
		...
		PreparedStatement statement = dbController.prepareSafeStatement("insert into "+tableName+" (USER_ID, EYE_LEVEL, ADD_DATA) values(?,?,?)");
		statement.setString(1, userId);
